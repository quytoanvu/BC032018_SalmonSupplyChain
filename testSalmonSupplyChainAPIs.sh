...
echo "POST request Enroll on Fredrick Org  ..."
echo
FREDRICK_ORG_TOKEN=$(curl -s -X POST \
  http://localhost:4000/users \
  -H "content-type: application/x-www-form-urlencoded" \
  -d 'username=Jim&orgName=Fredrick')
echo $FREDRICK_ORG_TOKEN
FREDRICK_ORG_TOKEN=$(echo $FREDRICK_ORG_TOKEN | jq ".token" | sed "s/\"//g")
echo
echo "FREDRICK_ORG token is $FREDRICK_ORG_TOKEN"
echo
